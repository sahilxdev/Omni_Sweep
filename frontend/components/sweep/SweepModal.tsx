...
export function SweepModal({ token, chain, chainId, worldIdVerified, onClose }: SweepModalProps) {
  ...
  const fetchQuote = async () => {
    ...
    try {
      ...
      const { fetchQuote: getQuote } = await import('@/lib/api');
      
      // Call backend API for real quote
      ...
      const formattedQuote: QuoteResponse = {
        oneInchData: quoteData.oneInchData,
        pythUpdateData: [], // Backend handles this
        estUsdcOut: quoteData.estimatedOutput,
        estFeeUsd: 0.15, // Estimate from quote if available
      };
      
      setQuote(formattedQuote);
      toast.success(`Quote: ${(parseInt(quoteData.estimatedOutput) / 1e6).toFixed(2)} USDC`);
    } catch (error) {
